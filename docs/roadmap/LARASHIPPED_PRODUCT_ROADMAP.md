...
| 2026-08-28 | Complete the reporting and moderation half of Milestone 0.2 with builder-filed content reports and a curator resolution queue. | The same audit found reporting, moderation, and duplicate-handling review paths entirely absent while comments, reviews, and cheers were already live — the community-safety contract ("reporting … before community features expand") was already violated, leaving the trust differentiator unprotected during the 20-builder cohort. | Any signed-in builder can report a project, comment, or review with a curated reason; curators resolve with an explicit outcome (no action / action taken) that never auto-enforces; `content_report_submitted` / `content_report_resolved` events recorded server-side. Experiment record opened at `docs/roadmap/experiments/2026-08-28-content-reports.md`. Public counters, penalties, and automated takedowns stay excluded. | First cohort check-in reviews report volume, time-to-resolution, and outcome mix. |
