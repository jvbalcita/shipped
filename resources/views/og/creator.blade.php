@php
    ...
    $nameFontSize = $nameNeedsFit ? 76 : 92;
    $titleNeedsFit = mb_strlen($title) > 60;
    $bioNeedsFit = mb_strlen($bio) > 72;
@endphp
<svg width="1200" height="630" viewBox="0 0 1200 630" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="{{ $creator->name }} Shipping Profile">
    ...
    <text x="94" y="462" font-family="Arial, Helvetica, sans-serif" font-size="30" @if ($titleNeedsFit) textLength="1016" lengthAdjust="spacingAndGlyphs" @endif fill="#585852">{{ $title }}</text>
    @if ($bio)
        <text x="94" y="504" font-family="Arial, Helvetica, sans-serif" font-size="22" @if ($bioNeedsFit) textLength="1016" lengthAdjust="spacingAndGlyphs" @endif fill="#585852">{{ $bio }}</text>
    @endif
