@php
    ...
    $nameNeedsFit = mb_strlen($name) > 24;
    $nameFontSize = $nameNeedsFit ? 84 : 104;
    $taglineNeedsFit = mb_strlen($tagline) > 60;
    $taglineFontSize = $taglineNeedsFit ? 28 : 32;
    $storyNeedsFit = $storyExcerpt !== null && mb_strlen($storyExcerpt) > 72;
@endphp
<svg width="1200" height="630" viewBox="0 0 1200 630" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="{{ $project->name }}">
    ...
    {{-- Project name --}}
    <text x="92" y="402" font-family="Arial Black, Arial, Helvetica, sans-serif" font-weight="900" font-size="{{ $nameFontSize }}" @if ($nameNeedsFit) textLength="1016" lengthAdjust="spacingAndGlyphs" @endif letter-spacing="-4" fill="#050505">{{ $name }}</text>

    {{-- Tagline --}}
    @if ($tagline)
        <text x="94" y="468" font-family="Arial, Helvetica, sans-serif" font-size="{{ $taglineFontSize }}" @if ($taglineNeedsFit) textLength="1016" lengthAdjust="spacingAndGlyphs" @endif fill="#585852">{{ $tagline }}</text>
    @endif

    @if ($storyExcerpt)
        <text x="94" y="508" font-family="Arial, Helvetica, sans-serif" font-size="22" @if ($storyNeedsFit) textLength="1016" lengthAdjust="spacingAndGlyphs" @endif fill="#585852">{{ $storyExcerpt }}</text>
    @endif
