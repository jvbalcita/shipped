@php
    ...
    $titleNeedsFit = mb_strlen($title) > 24;
    $titleFontSize = $titleNeedsFit ? 78 : 96;
    $projectNameNeedsFit = mb_strlen($projectName) > 24;
    $notesNeedsFit = mb_strlen($notes) > 60;
    $notesFontSize = $notesNeedsFit ? 28 : 32;
@endphp
<svg width="1200" height="630" viewBox="0 0 1200 630" fill="none" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="{{ $release->title }} — {{ $release->project->name }}">
    ...
    <text x="1108" y="118" text-anchor="end" font-family="Arial, Helvetica, sans-serif" font-size="26" @if ($projectNameNeedsFit) textLength="1016" lengthAdjust="spacingAndGlyphs" @endif font-weight="700" letter-spacing="3" fill="#050505">{{ $projectName }}</text>
    ...
    <text x="92" y="402" font-family="Arial Black, Arial, Helvetica, sans-serif" font-weight="900" font-size="{{ $titleFontSize }}" @if ($titleNeedsFit) textLength="1016" lengthAdjust="spacingAndGlyphs" @endif letter-spacing="-4" fill="#050505">{{ $title }}</text>
    @if ($notes)
        <text x="94" y="468" font-family="Arial, Helvetica, sans-serif" font-size="{{ $notesFontSize }}" @if ($notesNeedsFit) textLength="1016" lengthAdjust="spacingAndGlyphs" @endif fill="#585852">{{ $notes }}</text>
    @endif
